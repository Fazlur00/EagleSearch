	...
	foreach ($linklist as $link) {
		...
		if (!in_array($l, $already_crawled)) {
				...
				if($rows>0){
					...
					$result=$pdo->prepare("UPDATE`index` SET title=:title,description=:description,keywords=:keywords,url=:url,hash_url=:hash_url WHERE hash_url=:hash_url");
					$result=$result->execute($params);
					}
				}else{
					if(!is_null($params[':title']) && !is_null($params[':description']) && $params[':title'] != ''){
					$result=$pdo->prepare("INSERT INTO `index` VALUES('',:title,:description,:keywords,:url,:hash_url)");
					$result=$result->execute($params);
					}
				}
		}
